...
# Приведение к стационарности двойным дифференцированием
df['Pass_diff'] = df['Passengers'].diff().diff()
...
with t1:
    with st.container(width=900):
        ...
        with col1:
            st.write('📶🛠️Данные после двойного дифференцирования')
            st.write(df)
        with col2:
...
fig.update_layout(title='📈🛠️Приведение временного ряда к стационарности двойным дифференцированием',
                  xaxis_title="Дата",
