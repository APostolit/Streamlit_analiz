...
with st.container(width=600):
    ...
    # Перевести ВВП из долларов США в триллионы $
    df_pivot = df_pivot / 1e12
    st.subheader('🏦Валовый внутренний продукт по данным из API Всемирного банка, (трлн.$)')
    st.write('📆Сводная таблица за период с ', d1_str, ' по ', d2_str)
